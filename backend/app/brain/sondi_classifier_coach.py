...
def fetch_data_from_database(table_name):
    ...
    cursor = conn.cursor()
    cursor.execute(f"SELECT photo_name, style_id FROM {table_name}")
    data = cursor.fetchall()
    conn.close()

    # Bildpfade erstellen und der Datenliste hinzufügen
    data_with_image_path = [(create_image_path(photo_name, table_name), style_id - 1) for photo_name, style_id in data]  # Reduzieren Sie das Label um 1
    return data_with_image_path

# Funktion zum Erstellen des Dateipfads zu den Bildern
def create_image_path(photo_name, table_name):
    # Navigiere ein Verzeichnis höher, dann in 'Datenbanken' und dann in 'pics'
    pics_dir = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'Datenbank', 'pics'))
    # Extrahiere den Teil des Tabellennamens nach dem Bindestrich und füge 'pics' hinzu
    collection_name = table_name.split('_')[0] + '_pics'
    # Verzeichnis für die Bilder der spezifischen Collection
    collection_pics_dir = os.path.join(pics_dir, collection_name)
    # Dateipfad zum Bild
    image_path = os.path.join(collection_pics_dir, photo_name)
    return image_path

# Trainingsfunktion für ein spezifisches Modell
def train_sondi_model(table_name):
    # Modellpfad basierend auf dem Tabellennamen festlegen
    model_path = f'{table_name}_model.pth'

    # Trainiere das Modell mit den Daten aus der Datenbank und speichere es ab
    train_model(fetch_data_from_database(table_name), model_path)
